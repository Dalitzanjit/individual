...
def index():
    user = {'username': 'Dalitza MBL Analysis'}
...
def api_add() -> str:
    ...
    cursor = mysql.get_db().cursor()
    inputData = (content['Name'], request.form.get('Team'), request.form.get('Position'), request.form.get('Height'),
                 request.form.get('Weight'), request.form.get('Age'))
    sql_insert_query = """INSERT INTO Players (Name, Team, Position, Height_inches, Weight_lbs, Age) Values 
    (%s, %s, %s, %s, %s, %s, %s)"""
    cursor.execute(sql_insert_query, inputData)
    mysql.get_db().commit()
